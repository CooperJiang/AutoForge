...
frontend-install: web-install ## 安装所有前端依赖（仅web端）

.PHONY: frontend-build
frontend-build: web-build ## 构建所有前端项目（仅web端）

.PHONY: frontend-clean
frontend-clean: web-clean ## 清理所有前端构建文件（仅web端）

.PHONY: frontend-lint
frontend-lint: web-lint ## 检查所有前端代码（仅web端）

.PHONY: frontend-format
frontend-format: web-format ## 格式化所有前端代码（仅web端）
...
status: ## 📊 查看服务状态
	...
	@lsof -i :7777 2>/dev/null | head -2 || echo "  端口 7777: 未占用"
	@lsof -i :3200 2>/dev/null | head -2 || echo "  端口 3200: 未占用"
	@echo ""
	@# 检查PID文件
...
fullstack-dev: ## 启动全栈开发环境 (并行启动前后端)
	...
	@echo "$(YELLOW)后端将在 :7777 端口启动$(RESET)"
	@echo "$(YELLOW)用户端将在 :3200 端口启动$(RESET)"
...
help: ## 显示帮助信息
	...
	@echo "  $(YELLOW)make start$(RESET)              # 🚀 一键启动开发环境 (系统终端)"
	@echo "  $(YELLOW)make start-bg$(RESET)           # 🚀 一键启动开发环境 (后台模式)"
