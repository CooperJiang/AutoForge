  ...
  <Teleport to="body">
    <Transition name="dialog">
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="onClose">
        ...
        <!-- 对话框内容 -->
        <div class="relative bg-bg-elevated rounded-lg shadow-xl border-2 border-border-primary w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col">
          <!-- 标题 -->
          <div class="px-5 py-4 border-b-2 border-border-primary flex items-center justify-between flex-shrink-0">
            ...
            <button @click="onClose" class="text-text-tertiary hover:text-text-secondary transition-colors">
              <X :size="20" />
            </button>
          </div>
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="bg-bg-hover border-2 border-border-primary rounded-lg p-3">
                  ...
                  <div class="text-lg font-semibold" :class="result?.status_code && result.status_code >= 200 && result.status_code < 300 ? 'text-emerald-600' : 'text-error'">
                    {{ result?.status_code || '-' }}
                  </div>
                </div>
                <div class="bg-bg-hover border-2 border-border-primary rounded-lg p-3">
